...
enum { M_VIEW = 0, M_MOVE = 1, M_BOARDMOVE = 2, M_BOARDBACK=3 };
static int ox = 0;
static int	oy = 0;
static int buttonState = 0;
static float camera_trans[3] = { 0, -32, -250 };
...
static void renderscene(){
	...
	//board collider
	glPushMatrix();
	float3 board = psystem->getboardPos();
	glTranslatef(board.x, board.y, board.z);
	...
	//buoyancy collider
	glPushMatrix();
	float3 buo = psystem->getBuoyancyPos();
	glTranslatef(buo.x, buo.y, buo.z);
	glColor4f(0.0, 1.0, 0.0, 1.0);
	glutSolidSphere(psystem->getBuoyancyRadius(), 50, 50);
	glPopMatrix();

	{
		renderer->beginPostRender();
		...
		glutSolidSphere(psystem->getColliderRadius(), 50, 50);
		glPopMatrix();
		//render buoyancy collider depth
		glPushMatrix();
		float3 buo = psystem->getBuoyancyPos();
		glTranslatef(buo.x, buo.y, buo.z);
		glColor4f(0.0, 1.0, 0.0, 1.0);
	...
	{//PARTICLE_FLUID  PARTICLE_SPHERES  PARTICLE_POINTS
...
static void motion(int x, int y)
{
	...
	if (mode == M_MOVE){
		...
		//float3 p = psystem->getColliderPos();
		float3 p = psystem->getBuoyancyPos();
